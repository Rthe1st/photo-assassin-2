...
export class MapData {
    ...
    constructor(gameState: SharedGame.ClientGame, map: google.maps.Map, snipeClickHandler: (title: string, imageId: number) => void, playerColours: string[]) {
    ...
    drawKalmanPath(playerPublicId: number, minTime: number, maxTime: number) {
        ...
        for (var position of positions) {
            ...
            let latlng: google.maps.ReadonlyLatLngLiteral = { lat: estLatitude, lng: estLongitude };
            path.push(latlng);
            if (position.timestamp! > maxTime) {
                break;
            }
    ...
    drawPlayerGpsPoints(playerPublicId: number, minTime: number, maxTime: number) {
        ...
        if (!(playerPublicId in this.points)) {
            ...
            for (var position of positions) {
                ...
                this.points[playerPublicId].push(new google.maps.Circle({
                    strokeColor: "#00FF00",
                    strokeOpacity: 0.8,
                    strokeWeight: 2,
                    fillColor: "#00FF00",
    ...
    drawSnipes(playerPublicId: number, clickHandler: (title: string, imageId: number) => void) {
